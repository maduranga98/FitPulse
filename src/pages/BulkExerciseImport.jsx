...
const BulkExerciseImport = () => {
  ...
  const validateExercise = (exercise, index) => {
    ...
    if (!exercise.categoryName || typeof exercise.categoryName !== "string") {
      ...
      if (!category) {
        errors.push(
          `Exercise ${index + 1}: Category "${exercise.categoryName}" not found`
        );
      }
    }
  ...
  const handleBulkImport = async () => {
    ...
    if (
      !window.confirm(
        `Import ${validExercises.length} exercises to the selected gym?`
      )
    ) {
      return;
    }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {previewMode && exercises.length > 0 && (
          <div className="bg-gray-800 rounded-lg p-6 mb-6">
            ...
            <div className="mb-4 p-4 bg-gray-700 rounded-lg">
              <p className="text-sm">
                Total: {exercises.length} | Valid:{" "}
                <span className="text-green-500">
                  {exercises.filter((ex) => ex.isValid).length}
                </span>{" "}
                | Invalid:{" "}
                  ...
                  {exercises.map((exercise, idx) => (
                    <tr
                      ...
                      className={`border-b border-gray-700 ${
                        !exercise.isValid ? "bg-red-900 bg-opacity-20" : ""
                      }`}
                    >
            ...
            <div className="mt-6 flex gap-4">
              <button
                ...
                disabled={
                  importing || exercises.filter((ex) => ex.isValid).length === 0
                }
              ...
              <div>
                ...
                <div className="bg-red-900 bg-opacity-20 p-4 rounded-lg max-h-60 overflow-y-auto">
                  {errors.map((error, idx) => (
                    <p key={idx} className="text-sm text-red-400 mb-1">
                      {error}
                    </p>
                  ))}
                </div>
        ...
        <div className="bg-gray-800 rounded-lg p-6">
          ...
          <ol className="list-decimal list-inside space-y-2 text-gray-300">
            ...
            <li>
              Make sure the gym has the required exercise categories created
            </li>
            <li>Download the sample JSON file to see the required format</li>
            <li>
              Prepare your JSON file with exercise data (can be an array of
              exercises or a single exercise)
            </li>
          ...
          <div className="mt-4 p-4 bg-yellow-900 bg-opacity-20 rounded-lg">
            ...
            <ul className="list-disc list-inside space-y-1 text-sm text-gray-300">
              <li>
                The categoryName must match an existing category name for the
                selected gym
              </li>
              ...
              <li>
                photoURLs and videoURLs are optional but must be arrays if
                provided
              </li>
            </ul>
          </div>
